...
def regroup_pages(image_paths, prefix, max_height=MAX_PAGE_HEIGHT):
    ...
    # Joriy yig'ilayotgan bo'lak (PIL Image obyektlari ro'yxati emas,
    # balki tasvirlar pastdan ulanadi, shuning uchun canvas asta-sekin quriladi)
    current_canvas = None  # Image obyekt yoki None
    current_height = 0

    def flush_canvas():
        nonlocal current_canvas, current_height, out_index
    ...
    def append_to_canvas(img):
        """img'ni joriy canvasga pastdan qo'shadi (canvas balandligini o'stiradi)."""
        nonlocal current_canvas, current_height
        if current_canvas is None:
            current_canvas = Image.new("RGB", (target_width, img.height), "white")
            current_canvas.paste(img, (0, 0))
            current_height = img.height
        else:
            new_canvas = Image.new("RGB", (target_width, current_height + img.height), "white")
            new_canvas.paste(current_canvas, (0, 0))
            new_canvas.paste(img, (0, current_height))
