...
# Cloudflare va Anti-bot himoyasini aylanib o'tish uchun
...
# =========================================================
# HTTP SESSION CONFIG (Chrome Emulyatsiyasi bilan)
# =========================================================
...
def make_session(referer=None):
    # impersonate="chrome120" moduli brauzerni haqiqiy kompyuterdagidek ko'rsatadi
...
def get_page_info(chapter_url):
    ...
    # 1. Reader ichidagi JS massivlarini izlash (pages = [...] yoki images = [...])
    ...
    # 2. HTML ichidan barcha rasm havolalarini izlash
    if not image_urls:
...
def download_one(item):
    ...
    # Anti-bot va poster rasmlarni tashlab yuborish (40 KB dan kichiklari)
    if len(response.content) < 40 * 1024:
        raise RuntimeError("Rasm juda kichik (xavfsizlik posteri yoki reklama)")
...
def process_url(chat_id, url):
    ...
    try:
        ...
        if not image_urls:
            bot.send_message(chat_id, "❌ Asl manhwa rasmlari topilmadi.")
            return
        ...
        bot.send_message(
            chat_id,
            f"📚 **{name}**\n\n🖼 {len(image_urls)} ta rasm tahlil qilinmoqda...\n⚡ Yuklanmoqda...",
            parse_mode="Markdown"
        )
        ...
        if not paths:
            bot.send_message(chat_id, "❌ Manhwa sahifalari yuklanmadi. Rasmlar anti-bot filtridan o'tmadi.")
            return
