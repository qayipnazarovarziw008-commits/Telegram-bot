...
# KRITIK TUZATISH: threaded=False qilamiz (Gunicorn bilan urishmasligi uchun)
...
# =========================================================
# CHAPTER HTML VA RASMLARNI YIG'ISH
# =========================================================
    ...
    for tag in soup.find_all(["img", "source"]):
        for attr in attributes:
            ...
            if val and not val.strip().startswith("data:image"):
                urls.append(urljoin(chapter_url, val.strip()))
        ...
        if srcset:
            for item in srcset.split(","):
                ...
                if val and not val.startswith("data:image"):
                    urls.append(urljoin(chapter_url, val))
...
def process_url(chat_id, url):
    ...
    try:
        ...
        if not paths:
            bot.send_message(chat_id, "❌ Hech qanday rasm yuklab bo'lmadi.")
            return
...
def webhook():
    if request.headers.get("content-type") == "application/json":
        ...
        # TUZATILGAN: Updates to'g'ridan-to'g'ri qayta ishlanadi
